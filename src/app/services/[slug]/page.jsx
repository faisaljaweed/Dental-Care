...
import { ChevronRight, CheckCircle2, ArrowRight, TrendingDown } from "lucide-react";
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 items-center">
            <div>
              <nav className="flex items-center gap-2 text-sm text-white/50 mb-6 flex-wrap" aria-label="Breadcrumb">
                ...
                <Link href="/services" className="hover:text-teal-light transition-colors">Services</Link>
                <ChevronRight size={14} />
                <span className="text-teal-light font-medium">{service.name}</span>
              </nav>
              ...
              <h1 className="text-4xl sm:text-5xl font-semibold text-white mb-5">{service.name}</h1>
              <p className="text-lg text-white/70 leading-relaxed mb-9">{service.heroLine}</p>
            ...
            {service.pains.map((p, i) => (
              <Reveal key={i} delay={i * 100}>
                <div className="rounded-2xl border border-line bg-ice p-8 h-full">
                  ...
                  <p className="text-[14.5px] text-muted leading-relaxed">{p.text}</p>
                </div>
              </Reveal>
            ))}
          </div>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-14 lg:gap-20">
            <Reveal>
              <div>
                ...
                <p className="text-[16px] text-muted leading-relaxed">{service.solution}</p>
              </div>
            </Reveal>
            <Reveal delay={120}>
